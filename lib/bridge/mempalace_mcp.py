...
class MemPalaceMCPTools:
    ...
    def _init_vector_search(self):
        ...
        try:
            ...
            # 尝试使用多语言模型 (paraphrase-multilingual-MiniLM-L12-v2)
            try:
                ...
                multilingual_model_path = os.path.expanduser(
                    "~/.cache/chroma/onnx_models/paraphrase-multilingual-MiniLM-L12-v2/onnx"
                )
                
                if os.path.exists(multilingual_model_path + "/model.onnx"):
                    
                    class MultilingualMiniLM_EmbeddingFunction(EmbeddingFunction[Documents]):
                        """自定义多语言嵌入函数，支持中文"""
                        ...
                        def __init__(self, model_path: str, preferred_providers: Optional[List[str]] = None):
                            ...
                            try:
                                ...
                                raise ValueError("onnxruntime not installed")
                            try:
                                self.Tokenizer = importlib.import_module("tokenizers").Tokenizer
                            except ImportError:
                                raise ValueError("tokenizers not installed")
                        ...
                        def tokenizer(self) -> Any:
                            ...
                            tokenizer.enable_padding(pad_id=0, pad_token="", length=256)
                            return tokenizer
                        ...
                        def _normalize(self, v) -> Any:
                            import numpy as np
                            norm = np.linalg.norm(v, axis=1)
                            norm[norm == 0] = 1e-12
                            return v / norm[:, np.newaxis]
                        
                        def _forward(self, documents: List[str], batch_size: int = 32) -> Any:
                            import numpy as np
                            all_embeddings = []
                    ...
                    self._embedding_fn = MultilingualMiniLM_EmbeddingFunction(multilingual_model_path)
                    model_name = "paraphrase-multilingual-MiniLM-L12-v2 (多语言, 支持中文)"
                    model_dim = 384
                else:
                    raise FileNotFoundError("多语言模型未找到")
            except Exception as e:
                # 回退到默认英文模型
                from chromadb.utils.embedding_functions import ONNXMiniLM_L6_V2
                self._embedding_fn = ONNXMiniLM_L6_V2()
                model_name = "ONNXMiniLM-L6-v2 (英文, 384维)"
                model_dim = 384
                print(f"[MemPalaceMCP] 多语言模型加载失败，回退到英文模型: {e}")
            
            self._chroma_collection = self._chroma_client.get_or_create_collection(
                name="wukong_memories",
                embedding_function=self._embedding_fn,
                metadata={"description": "悟空记忆向量库", "model": model_name}
            )
            print(f"[MemPalaceMCP] 向量搜索已就绪，模型: {model_name} ({model_dim}维)")
        except ImportError:
